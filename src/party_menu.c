...
static u8 CanMonLearnTMTutor(struct Pokemon *mon, u16 item, u8 tutor)
{
    ...
    if (item >= ITEM_TM_HONE_CLAWS)
    {
        if (!CanMonLearnTMHM(mon, item - ITEM_TM_HONE_CLAWS))
...
{
    u16 tmNumber = item - ITEM_TM_HONE_CLAWS;
...
static void Task_AbilityCapsule(u8 taskId)
{
	...
	case 2:
		...
		case MENU_B_PRESSED:
			...
			PlaySE(SE_SELECT);
			ScheduleBgCopyTilemapToVram(2);
			// Don't exit party selection screen, return to choosing a mon
...
static void Task_AbilityPatch(u8 taskId)
{
	...
	case 0:
		// can't use
		if (gSpeciesInfo[tSpecies].abilities[tAbilityNum] == ABILITY_NONE
			|| !tSpecies)
		{
			gPartyMenuUseExitCallback = FALSE;
			PlaySE(SE_SELECT);
...
static void Task_Mints(u8 taskId)
{
    ...
    case 2:
        ...
        case MENU_B_PRESSED:
            ...
            // Don't exit party selection, return to choosing a mon.
            ClearStdWindowAndFrameToTransparent(6, 0);
            ClearWindowTilemap(6);
            DisplayPartyMenuStdMessage(PARTY_MSG_USE_ON_WHICH_MON);
